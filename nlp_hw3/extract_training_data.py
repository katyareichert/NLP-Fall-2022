...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        # Returns: [  stack[-1], stack[-2], stack[-3],     buffer[-1], buffer[-2], buffer[-3] ]

        input_rep = []

        words_copy = words[:] 
        stack_copy = state.stack[:]
        buff_copy = state.buffer[:]

        while len(stack_copy) < 3:
            stack_copy.insert(0, len(words))
        while len(buff_copy) < 3:
            buff_copy.insert(0, len(words))
        words_copy.append('<NULL>')
        ...
        for j in range(2):
            ...
            for i in range(-1,-4,-1):
                ...
                if word in self.word_vocab:
                    input_rep.append(self.word_vocab[word])
                elif isinstance(word, str) and word.lower() in self.word_vocab:
                    input_rep.append(self.word_vocab[word.lower()])
                elif not index:
        ...
        assert(len(input_rep) == 6)
        return np.array(input_rep)

    def get_output_representation(self, output_pair):  
        # output_pair: (transition, label)
        # Returns: one-hot vector of length 91 encoding the output pair
